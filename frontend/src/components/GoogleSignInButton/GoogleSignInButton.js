import React, { useEffect, useRef } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import { useTheme } from '../../contexts/ThemeContext';
import './GoogleSignInButton.css';

const GoogleSignInButton = ({ onSuccess, text = "Continue with Google" }) => {
  const { handleGoogleResponse } = useAuth();
  const { theme } = useTheme();
  const buttonRef = useRef(null);

  useEffect(() => {
    if (window.google && window.google.accounts) {
      // Render the Google Sign-In button
      window.google.accounts.id.renderButton(
        buttonRef.current,
        {
          theme: theme === 'light' ? 'outline' : 'filled_blue',
          size: 'large',
          text: 'continue_with',
          shape: 'rectangular',
          width: '100%'
        }
      );
    }
  }, [theme]);

  const handleManualGoogleLogin = () => {
    if (!process.env.REACT_APP_GOOGLE_CLIENT_ID || process.env.REACT_APP_GOOGLE_CLIENT_ID === 'your_google_client_id_here') {
      alert(`Google Auth not configured yet!\n\nCurrent Client ID: ${process.env.REACT_APP_GOOGLE_CLIENT_ID}\n\nTo fix:\n1. Add your Google Client ID to the .env file\n2. Restart the development server\n3. Make sure the .env file is in the frontend folder`);
      return;
    }
    
    if (window.google && window.google.accounts) {
      console.log('Prompting Google Auth...');
      try {
        window.google.accounts.id.prompt((notification) => {
          console.log('Google prompt notification:', notification);
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            console.log('Prompt was not displayed or skipped');
          }
        });
      } catch (error) {
        console.error('Error with Google prompt:', error);
        alert('There was an issue with Google Auth. Please try again or use the demo login.');
      }
    } else {
      alert('Google Auth is still loading. Please try again in a moment.');
    }
  };

  return (
    <div className={`google-signin-container ${theme}`}>
      {/* Google's rendered button */}
      <div ref={buttonRef} className="google-button-container"></div>
      
      {/* Fallback manual button */}
      <button 
        onClick={handleManualGoogleLogin}
        className="google-signin-fallback"
        type="button"
      >
        <svg className="google-icon" viewBox="0 0 24 24" width="20" height="20">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
        {text}
      </button>
    </div>
  );
};

export default GoogleSignInButton;